...
if os.path.exists('test.db'):
    os.remove('test.db')
# tworzymy instalację klasy Engine do obsługi bazy
...
#__tablename__ wskazuje nazwę tabeli w bazie danych, pod którą chcemy przechowywać informację

#tworzymy sesję, która przechowyje obiekty i umożliwia "rozmowę" z bazą
BDSesja = sessionmaker(bind=baza)
sesja = BDSesja()

#dodajemy dwia klasy, jeżeli tabela jest pusta
if not sesja.query(Klasa).count():
    sesja.add(Klasa(nazwa='1A', profil='matematyczny'))
    sesja.add(Klasa(nazwa='1B', profil='humanistyczny'))
#tworzymy instalację klasy Klasa reprezentującą klasę "1A"
...
inst_uczen.klasa_id = sesja.query(Klasa.id).filter(Klasa.nazwa == '1B').scalar()
